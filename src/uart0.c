...
int32_t getFieldInteger(USER_DATA *data, uint8_t fieldNumber)
{
    if(fieldNumber <= MAX_FIELDS) {
        if(data->fieldType[fieldNumber] == 'n') {
            int32_t field = 0, i = 0;
            for(i = 0; data->buffer[data->fieldPosition[fieldNumber]+i] != '\0'; i++) {
                field = field * 10 + (data->buffer[data->fieldPosition[fieldNumber]+i] - '0');
            }
            return field;
        }
    }
...
bool isCommand(USER_DATA *data, const char strCommand[], uint8_t minArguments)
{
    if (minArguments < data->fieldCount) {
        uint8_t i = 0;
        bool valid = false;
        while(data->buffer[data->fieldPosition[0]+i]) {
